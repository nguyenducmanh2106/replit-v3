  ...
  async function handleCreate() {
    ...
    try {
      ...
      await queryClient.invalidateQueries({
        queryKey: [`/api/courses/${courseId}/schedule`],
      });
      ...
      toast({
        title: "Lỗi",
        description: "Không thể thêm lịch",
        variant: "destructive",
      });
    }
  }
    ...
    <div className="space-y-4">
      ...
      {(events ?? []).length === 0 ? (
        ...
        <div className="space-y-2">
          {(events ?? []).map((e: any) => (
            <div
              key={e.id}
              className="flex items-center justify-between px-4 py-3 border rounded-lg bg-white hover:bg-gray-50"
            >
              <div className="flex items-center gap-3">
                <div className="flex-shrink-0">
                  <Badge
                    className={`text-xs border-0 ${EVENT_TYPE_COLORS[e.type] ?? "bg-gray-100"}`}
                  >
      ...
      <Dialog open={showAdd} onOpenChange={setShowAdd}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Thêm lịch học/thi</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <div className="space-y-1.5">
              ...
              <Input
                ...
                placeholder="VD: Buổi học Unit 3"
              />
            </div>
            <div className="space-y-1.5">
              <Label>Loại</Label>
...
function DocumentsTab({
  ...
  async function handleAddDoc() {
    ...
    if (uploadMode === "file") {
      if (!selectedFile) {
        toast({
          title: "Vui lòng chọn file để tải lên",
          variant: "destructive",
        });
        return;
      }
  ...
  return (
    <div className="space-y-4">
      ...
      <Dialog open={showAdd} onOpenChange={setShowAdd}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Thêm tài liệu khoá học</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <div className="space-y-1.5">
  ...
  async function handleImport() {
    ...
    try {
      const res = await importMembers({
        id: courseId,
        data: { csvData, role: defaultRole },
      });
      setResult(res);
      await queryClient.invalidateQueries({
        queryKey: [`/api/courses/${courseId}/members`],
      });
      toast({ title: `Nhập thành công ${res.imported} thành viên` });
    } catch {
      toast({
        title: "Lỗi",
        description: "Không thể nhập dữ liệu",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
    ...
    <div className="space-y-5 max-w-2xl">
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        ...
        <p className="text-xs text-blue-600">
          File CSV cần có cột: email, name (tuỳ chọn), role (tuỳ chọn). Người
          dùng phải đã có tài khoản trong hệ thống.
        </p>
        <button
      ...
      {result && (
        <div
          className={`p-4 rounded-lg border ${result.errors.length > 0 ? "border-amber-200 bg-amber-50" : "border-green-200 bg-green-50"}`}
        >
          <p className="text-sm font-medium">Kết quả nhập:</p>
          <p className="text-sm mt-1">
            ✅ Đã nhập: <strong>{result.imported}</strong> · ⏭️ Bỏ qua:{" "}
  ...
  async function handleCreate() {
    ...
    try {
      ...
      queryClient.invalidateQueries({
        queryKey: getListAssignmentsQueryKey({ courseId }),
      });
      ...
      toast({
        title: err?.message || "Lỗi tạo bài tập",
        variant: "destructive",
      });
    }
  }
    ...
    <div className="space-y-4">
          ...
          {assignments.map((a) => {
            const cardContent = (
              <Card
                className={
                  canManage
                    ? "cursor-pointer hover:shadow-sm hover:border-primary/30 transition-all"
                    : ""
                }
              >
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-3 mt-0.5">
                          ...
                          <span className="text-xs text-muted-foreground">
                            {a.questionCount} câu — {a.totalPoints} điểm
                          </span>
                        </div>
                      </div>
                    </div>
      ...
      >
        <DialogContent>
                ...
                {retakeWarning.maxAttempts > 0 ? (
                  <p className="text-sm text-amber-800">
                    ...
                    <span className="font-bold">
                      {retakeWarning.myAttemptCount + 1}
                    </span>{" "}
                    trên giới hạn{" "}
                    <span className="font-bold">
                      {retakeWarning.maxAttempts}
                    </span>{" "}
                    lần cho phép.
                  </p>
                ) : (
                  <p className="text-sm text-amber-800">
                    Bạn đã làm bài này{" "}
                    <span className="font-bold">
                      {retakeWarning.myAttemptCount}
                    </span>{" "}
                    lần trước đó.
                  </p>
                )}
                <p className="text-sm font-medium text-amber-800">
                  Nếu đồng ý làm lại, kết quả của lần làm mới nhất sẽ được ghi
                  nhận. Số lần đã làm vẫn được tính cộng dồn.
                </p>
              </div>
              {retakeWarning.maxAttempts > 0 && (
                <p className="text-xs text-muted-foreground">
                  Còn lại:{" "}
                  {retakeWarning.maxAttempts - retakeWarning.myAttemptCount}{" "}
                  lượt
                </p>
              )}
            </div>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setRetakeWarning(null)}>
              Hủy
            </Button>
            <Button
              variant="destructive"
              onClick={() => {
                if (retakeWarning)
                  navigate(`/assignments/${retakeWarning.id}/take`);
...
export default function CourseDetailPage() {
  ...
  const { data: members, isLoading: loadingMembers } = useGetCourseMembers(
    courseId,
    {
      query: {
        enabled: !!courseId,
        queryKey: [`/api/courses/${courseId}/members`] as const,
      },
    },
  );
  const { data: assignments, isLoading: loadingAssignments } =
    useListAssignments({ courseId });
      ...
      <Tabs defaultValue={canManage ? "members" : "assignments"}>
        <TabsList>
          ...
          <TabsTrigger value="assignments">
            Bài tập ({assignments?.length ?? 0})
          </TabsTrigger>
          <TabsTrigger value="schedule">Lịch học</TabsTrigger>
          <TabsTrigger value="documents">Tài liệu</TabsTrigger>
          {canManage && (
            <TabsTrigger value="import">Nhập danh sách</TabsTrigger>
          )}
        </TabsList>
