...
const TYPE_LABELS: Record<string, string> = {
  ...
  word_selection: "Chọn từ",
  matching: "Nối cặp",
  drag_drop: "Kéo thả",
  sentence_reorder: "Sắp xếp câu",
  reading: "Đọc hiểu",
  listening: "Nghe hiểu",
...
export default function QuestionsPage() {
  ...
  const typeCounts = Array.isArray(allQuestions)
    ? (allQuestions ?? []).reduce<Record<string, number>>(
        ...
        <div className="relative flex items-center justify-between">
          <div>
            ...
            <p className="text-blue-200 text-sm mt-1">
              Tạo và tái sử dụng câu hỏi cho tất cả bài thi
            </p>
          </div>
            ...
            <div className="flex h-2 rounded-full overflow-hidden gap-0.5">
              {TYPES.filter((t) => typeCounts[t] > 0).map((t) => {
                ...
                return (
                  <div
                    key={t}
                    className={`bg-gradient-to-r ${cfg.gradient} opacity-80 transition-all`}
            ...
            <div className="flex flex-wrap gap-x-3 gap-y-1 mt-2">
              {TYPES.filter((t) => typeCounts[t] > 0).map((t) => (
                <span key={t} className="text-xs text-blue-100">
                  {TYPE_LABELS[t]}:{" "}
                  <strong className="text-white">{typeCounts[t]}</strong>
                </span>
              ))}
            </div>
          </div>
        ...
        <div className="space-y-3">
          {questions.map((q: any) => {
            ...
            return (
              <Card
                key={q.id}
                className={`group hover:shadow-md transition-all border-2 ${cfg.border} hover:scale-[1.005]`}
              >
                <CardContent className="py-4 px-5">
                  <div className="flex items-start gap-4">
                    <div
                      className={`flex-shrink-0 w-10 h-10 rounded-xl bg-gradient-to-br ${cfg.gradient} text-white flex items-center justify-center shadow-sm`}
                    >
                      {cfg.icon}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-wrap items-center gap-2 mb-1.5">
                        <span
                          className={`text-xs font-semibold px-2 py-0.5 rounded-full ${cfg.light} ${cfg.text}`}
                        >
                      ...
                      {options.length > 0 && isCorrectable && (
                        <div className="mt-2 flex flex-wrap gap-1.5">
                          {options.slice(0, 4).map((opt: string, i: number) => (
                            <span
                              key={i}
                              className={`text-xs px-2 py-1 rounded-lg font-medium flex items-center gap-1 ${opt === q.correctAnswer ? "bg-green-100 text-green-700 border border-green-200" : "bg-gray-50 text-gray-500 border border-gray-100"}`}
                            >
                              <span
        ...
        <div className="text-center py-20 border-2 border-dashed border-gray-200 rounded-2xl">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {search
              ...
              : 'Nhấn "Tạo câu hỏi mới" hoặc "Import Excel" để bắt đầu'}
          </p>
          {!search && (
      ...
      >
        <DialogContent className="max-w-sm">
          ...
          <p className="text-sm text-muted-foreground">
            Bạn có chắc muốn xoá câu hỏi này? Thao tác này không thể hoàn tác.
          </p>
      ...
      >
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              Tải lên file Excel (.xlsx) với mỗi sheet tương ứng một dạng câu
              hỏi
            </DialogDescription>
          </DialogHeader>
          ...
          {!importResult ? (
            <div className="space-y-4">
              <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
                ...
                <ul className="text-xs text-blue-700 space-y-1.5">
                  <li>
                    Mỗi <strong>sheet</strong> tương ứng 1 dạng câu hỏi: MCQ,
                    TRUE_FALSE, FILL_BLANK, MATCHING, SENTENCE_REORDER, ESSAY,
                    OPEN_END, WORD_SELECTION, DRAG_DROP, READING, LISTENING
                  </li>
                  <li>
                    Các cột chung:{" "}
                    <code className="bg-blue-100 px-1 rounded">content</code>,{" "}
                    <code className="bg-blue-100 px-1 rounded">skill</code>,{" "}
                    <code className="bg-blue-100 px-1 rounded">level</code>,{" "}
                    <code className="bg-blue-100 px-1 rounded">points</code>,{" "}
            ...
            <div className="space-y-4">
              <div
                className={`rounded-xl p-4 ${importResult.totalImported > 0 ? "bg-green-50 border border-green-200" : "bg-amber-50 border border-amber-200"}`}
              >
                  ...
                  <div>
                    ...
                    {importResult.totalSkipped > 0 && (
                      <p className="text-xs text-gray-500">
                        {importResult.totalSkipped} dòng bị bỏ qua
                      </p>
                    )}
                  </div>
                </div>
