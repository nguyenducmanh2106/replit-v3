...
function StatusBadge({ status }: { status: string }) {
  if (status === "published")
    return (
      <Badge className="bg-green-100 text-green-700 border-0">Đã công bố</Badge>
    );
  if (status === "closed")
    return (
      <Badge className="bg-gray-100 text-gray-500 border-0">Đã đóng</Badge>
    );
      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
          {assignments.map((a: any) => {
            ...
            const exceededLimit =
              !isTeacher && maxAttempts > 0 && myAttempts >= maxAttempts;
                ...
                <Link
                  href={
                    isTeacher
                      ...
                      : exceededLimit
                        ? "#"
                        : `/assignments/${a.id}/take`
                  }
                  onClick={(e: React.MouseEvent) => {
                    if (exceededLimit) e.preventDefault();
                  }}
                >
                  <Card
                    className={`cursor-pointer hover:shadow-md hover:border-primary/30 transition-all ${exceededLimit ? "opacity-60 cursor-not-allowed" : ""}`}
                  >
        ...
        <div className="text-center py-20 border-2 border-dashed border-gray-200 rounded-2xl">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {isTeacher
              ? 'Nhấn "Tạo bài tập" để bắt đầu'
              : "Bạn chưa được gán vào khoá học nào có bài tập đang mở"}
          </p>
        </div>
      )}
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 pt-2">
            <div>
              ...
              <Input
                ...
                placeholder="VD: Kiểm tra giữa kỳ — Tiếng Anh B2"
                className="mt-1"
              />
              {errors.title && (
