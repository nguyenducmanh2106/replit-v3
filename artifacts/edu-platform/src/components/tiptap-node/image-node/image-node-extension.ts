...
export const Image = TiptapImage.extend<ImageOptions>({
  ...
  addKeyboardShortcuts() {
    // Helper: locate an image node based on the current selection. Returns the
    // image node, its position, and whether the selection is at the start/end
    // of the image's inline content (or a NodeSelection on the image itself).
    const resolveImageFromSelection = (
    ...
    return {
      ...
      // Because the Image node has `content: "inline*"` (to support captions)
      // it is treated as a textblock, which means ProseMirror's Gapcursor does
      // not activate before/after it. If the image is the first or last child
      // of the document there is no way to place the cursor above/below it.
      // These handlers detect that situation and insert an empty paragraph at
      // the boundary so the caret can naturally move outside of the image —
      // giving us the "atom-like" caret behaviour while keeping the caption.
      ArrowUp: ({ editor }) => {
        const { state } = editor
        const match = resolveImageFromSelection(state.selection, this.type)

        if (!match || !match.atStart) {
          return false
        }

        // Only intercept when the image is the first child of the document.
        if (match.imagePos !== 0) {
          return false
        }
