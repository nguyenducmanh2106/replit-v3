...
export const TYPE_CONFIG: Record<string, { label: string; icon: any; color: string; bgColor: string; gradient: string }> = {
...
// ─── True/False ────────────────────────────────────────────────────────────────
export function TrueFalseInput({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  const opts = [
...
// ─── Fill Blank ───────────────────────────────────────────────────────────────
export function FillBlankInput({ content, value, onChange }: { content: string; value: string; onChange: (v: string) => void }) {
...
// ─── Word Selection ───────────────────────────────────────────────────────────
export function WordSelectionInput({ options, value, onChange }: { options: string[]; value: string; onChange: (v: string) => void }) {
...
export const MATCHED_COLOR = { left: "#22C55E", leftLight: "#DCFCE7", right: "#16A34A", rightLight: "#D1FAE5" };

export function PuzzleLeft({ color, light, children, className, ...props }: {
...
export function PuzzleRight({ color, light, children, className, isDragging, ...props }: {
...
export function MatchingInput({ pairs, value, onChange }: { pairs: Array<{ left: string; right: string }>; value: string; onChange: (v: string) => void }) {
...
export function OpenEndInput({ allowedTypes, value, onChange }: { allowedTypes: string[]; value: string; onChange: (v: string) => void }) {
...
export function SentenceReorderInput({ items, value, onChange }: { items: string[]; value: string; onChange: (v: string) => void }) {
...
// ─── Reading (Split Pane) ─────────────────────────────────────────────────────
export function ReadingInput({ passage, options, value, onChange, questionId }: { passage?: string | null; options?: unknown[]; value: string; onChange: (v: string) => void; questionId?: number | string }) {
...
// ─── Video Interactive ────────────────────────────────────────────────────────
export function getYouTubeIdTake(url: string): string | null {
  if (!url) return null;
  const m = url.match(/(?:youtu\.be\/|youtube\.com\/(?:watch\?v=|embed\/|v\/|shorts\/))([a-zA-Z0-9_-]{11})/);
  return m ? m[1] : null;
}

export function buildYTEmbedUrl(videoId: string): string {
  return `https://www.youtube.com/embed/${videoId}?enablejsapi=1&playsinline=1&rel=0`;
}

export function VideoInteractiveInput({ videoUrl, timedQuestions: rawTq, value, onChange }: { videoUrl?: string | null; timedQuestions: unknown[]; value: string; onChange: (v: string) => void }) {
...
// ─── Listening Input ──────────────────────────────────────────────────────────
export function ListeningInput({ audioUrl, options, value, onChange }: { audioUrl?: string | null; options: unknown[]; value: string; onChange: (v: string) => void }) {
  type SubQ = { question: string; choices: string[]; correctAnswer: string };
  const subQuestions: SubQ[] = useMemo(() => {
    if (!options || options.length === 0) return [];
    const first = options[0];
...
export function QuestionRenderer({
  q, value, onChange,
}: { q: RenderableQuestion; value: string; onChange: (v: string) => void }) {
  ...
  const wordSelectionWords: string[] = q.type === "word_selection" && q.passage
    ? String(q.passage).split(/\s+/).filter(Boolean)
    : rawOptions;

  const pairs = q.type === "matching"
    ? (rawOptionsArray as Array<{ left?: string; right?: string } | string>).map(item => {
        if (typeof item === "object" && item !== null) {
          return { left: String((item as any).left ?? ""), right: String((item as any).right ?? "") };
        }
        const s = String(item);
        const sepIdx = s.indexOf("|");
        return sepIdx >= 0
          ? { left: s.slice(0, sepIdx).trim(), right: s.slice(sepIdx + 1).trim() }
          : { left: s, right: "" };
      }).filter(p => p.left || p.right)
    : [];

  const ddParsed = safeJsonParse<{ items?: string[]; zones?: Array<{label:string;accepts:string[]}> }>(
    typeof q.options === "string" ? q.options : JSON.stringify(q.options ?? null),
    {}
  );
  const items = q.type === "drag_drop"
    ? (Array.isArray(ddParsed.items) ? ddParsed.items : rawOptions)
    : q.type === "sentence_reorder"
      ? rawOptions
      : [];

  if (q.type === "mcq" && rawOptions.length > 0 && !allowMultiple) {
    return (
      <div className="space-y-3">
        {rawOptions.map((opt, i) => (
          ...
          >
            <div className={cn(
              ...
              value === opt ? "bg-gradient-to-br from-blue-500 to-indigo-600 text-white shadow-md" : "bg-gray-100 text-gray-500"
            )}>
              {String.fromCharCode(65 + i)}
            </div>
            <input type="radio" name={`q-${q.id}`} value={opt} checked={value === opt} onChange={() => onChange(opt)} className="sr-only" />
            <span className={cn("text-sm font-medium flex-1 leading-relaxed", value === opt ? "text-blue-800" : "text-gray-700")}>{opt}</span>
            {value === opt && <CheckCircle2 className="w-5 h-5 text-blue-500 flex-shrink-0" />}
          </label>
        ))}
      </div>
    );
  ...
  if (q.type === "true_false") return <TrueFalseInput value={value} onChange={onChange} />;
  if (q.type === "fill_blank") return <FillBlankInput content={q.content ?? ""} value={value} onChange={onChange} />;
  if (q.type === "word_selection" && wordSelectionWords.length > 0)
    return <WordSelectionInput options={wordSelectionWords} value={value} onChange={onChange} />;
  if (q.type === "matching" && pairs.length > 0)
    return <MatchingInput pairs={pairs} value={value} onChange={onChange} />;
  if (q.type === "drag_drop" && items.length > 0)
    return <DragDropInput items={items} zones={ddParsed.zones ?? []} value={value} onChange={onChange} />;
  if (q.type === "sentence_reorder" && items.length > 0)
    return <SentenceReorderInput items={items} value={value} onChange={onChange} />;
  if (q.type === "reading")
    return <ReadingInput passage={q.passage} options={rawOptionsArray} value={value} onChange={onChange} questionId={q.id} />;
  if (q.type === "listening")
    return <ListeningInput audioUrl={q.audioUrl} options={rawOptionsArray} value={value} onChange={onChange} />;
  if (q.type === "video_interactive")
    return <VideoInteractiveInput videoUrl={q.videoUrl} timedQuestions={rawOptionsArray} value={value} onChange={onChange} />;
  if (q.type === "essay") {
    return (
      <div>
  ...
  if (q.type === "open_end") {
    const allowedTypes: string[] = (Array.isArray((meta as any).allowedTypes) ? (meta as any).allowedTypes : ["text", "audio"])
      .filter((t: string) => t !== "image");
    return <OpenEndInput allowedTypes={allowedTypes} value={value} onChange={onChange} />;
  }

  // Fallback (short_answer / long_answer / unknown)
  if (q.type === "long_answer" || q.type === "essay") {
    return (
      <textarea
        value={value}
        onChange={e => onChange(e.target.value)}
        ...
        className="w-full p-3 border-2 border-gray-200 focus:border-blue-400 rounded-xl text-sm resize-y"
      />
    );
  }
